...
def main(args):
    ...
    np.save("output/%s_title_embs.npy" % topic, title_embs.cpu().detach().numpy())
    np.save("output/%s_body_embs.npy" % topic, body_embs.cpu().detach().numpy())

    # Compute cosine similarities ahead of time
    cos_sims_title_title = util.cos_sim(title_embs, title_embs)[0]
    np.save("output/%s_cos_sims_title_title.npy" % topic, cos_sims_title_title.cpu().detach().numpy())

    cos_sims_title_body = util.cos_sim(title_embs, body_embs)[0]
    np.save("output/%s_cos_sims_title_body.npy" % topic, cos_sims_title_body.cpu().detach().numpy())

    cos_sims_body_body = util.cos_sim(body_embs, body_embs)[0]
    np.save("output/%s_cos_sims_body_body.npy" % topic, cos_sims_body_body.cpu().detach().numpy())
