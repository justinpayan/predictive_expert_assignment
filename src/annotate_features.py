...
def main(args):
    ...
    if feat_type == "answer_quality":
        ...
        if os.path.exists(outfilename):
            # Load in the pairs we already know so we don't repeat work
            with open(outfilename, 'r', encoding='utf-8') as f:
                r = csv.DictReader(f, delimiter='\t')
                for row in r:
                    known_pairs.add((row['QID'], row['AID']))
        else:
            # Set up the file so can just append to it.
            with open(outfilename, 'w', encoding='utf-8') as f:
                w = csv.writer(f, delimiter="\t")
                w.writerow(["QID", "AID", "Informativeness", "Relevance", "Usefulness", "Explanation"])

        print("We already know the features for this many pairs: ", len(known_pairs), flush=True)
        ...
        with open(outfilename, 'a', encoding='utf-8') as f:
            ...
            for qid, answers in question_id_to_answers.items():
                if qid in questions:
                    for a in answers:
                        aid = a['Id']
                        if (qid, aid) in known_pairs:
                            print("skipping ", qid, aid)
                        else:
